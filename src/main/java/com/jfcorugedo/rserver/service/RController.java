...
public class RController {
	...
	public ResponseEntity<List<int[]>> blockDiscrete(@RequestBody(required=false) List<String[]> data) {
		if(data != null) {
			return new ResponseEntity<>(rService.groupDiscreteValues(data), HttpStatus.CREATED);
		} else {
			throw new BadRequestException("This service needs all the values of the stratification variable");
		}
	}
	...
	public ResponseEntity<String> areGeneralizable(@RequestBody List<double[]> values) {
		...
		if(values.size() == 2) {
			...
			return new ResponseEntity<>(String.format("%s: %b", ksTest.getAlgorithm(), areGeneralizable), HttpStatus.CREATED);
		} else {
			return new ResponseEntity<>("This algorithm needs two different list of values", HttpStatus.BAD_REQUEST);
		}
	}
	...
	public ResponseEntity<Double> sqrt(@RequestBody Double number) {
		...
		return new ResponseEntity<>(rService.sqrt(number), HttpStatus.CREATED);
	}
}
